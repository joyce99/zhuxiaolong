...
lang = 'zh'
train_pair,dev_pair,adj_matrix,r_index,r_val,adj_features,rel_features,triples1,triples2,noisy1,noisy2,entityid1,entityid2,triples_pos1,triples_pos2 ,entity1,entity2,triplesinitial= load_data1('data/zh_en/',train_ratio=0.3)
...
ent_embding = tf.Variable(inputembeddings,trainable=True)
...
class TokenEmbedding1(keras.layers.Embedding):
    """Embedding layer with weights returned."""

    def built(self):
        self.embeddings=ent_embding
        self.built = True
...
def get_trgat(node_size, rel_size, node_hidden, rel_hidden, triple_size, n_attn_heads=2, dropout_rate=0, gamma=3,
    ...
    ent_emb = TokenEmbedding1(node_size, node_hidden, trainable=True)(val_input)
...
model, get_emb = get_trgat(dropout_rate=0.30, node_size=node_size, rel_size=rel_size, n_attn_heads=1, depth=1, gamma=3,
                           node_hidden=300, rel_hidden=100, triple_size=triple_size)
...
for i in trange(epoch):
    ...
    if i==epoch-1:
        ...
        net = Classfier1(2400,1200,600,300,150);
