...
lang = 'zh'
train_pair,dev_pair,adj_matrix,r_index,r_val,adj_features,rel_features,triples1,triples2,noisy1,noisy2,entityid1,entityid2,triples_pos1,triples_pos2,entity1,entity2,triplesinitial = load_data1('data/zh_en/',train_ratio=0.6)
...
#     np.random.shuffle(train_set);
#     train_set = train_set[:batch_size]
#     lenshape= train_set.shape[0]
#     nl=np.random.choice(list(entity2),(lenshape,1))
#     nr=np.random.choice(list(entity1),(lenshape,1))
#     sl = np.random.choice(list(entity1), (lenshape, 1))
#     sr = np.random.choice(list(entity2), (lenshape, 1))
#     for i in range(lenshape):
#         while nl[i][0]==train_set[i][1]:
#             k=np.random.choice(list(entity2))
#             nl[i][0]=k
#         while nr[i][0]==train_set[i][0]:
#             k=np.random.choice(list(entity1))
#             nr[i][0]=k
#         while sl[i][0]==train_set[i][0]:
#             k=np.random.choice(list(entity1))
#             sl[i][0]=k
#         while sr[i][0]==train_set[i][1]:
#             k=np.random.choice(list(entity2))
#             sr[i][0]=k
#     neg=np.concatenate([nl,nr,sl,sr],axis=-1)
#     train_set = np.concatenate([train_set, neg], axis=-1)
#     return train_set
...
net = Classfier(1200,600,300, 150,75);loss_function = nn.BCELoss();optimizer = optim.Adam(net.parameters(), lr=1e-3);
...
for i in trange(1200):
    ...
    if i==1199:
        ...
        lensorthalf1 = int(lensorthalf * 0.6);
        tempsorthalf = 0
        x=0;y=0;
        ...
        print("鉴别结束。。。")
        adj_matrix, r_index, r_val, rel_matrix, ent_matrix,adj_features,rel_features = update('data/dbp_yg/', triples1, triples2)
...
rest_set_1 = [e1 for e1, e2 in dev_pair]
rest_set_2 = [e2 for e1, e2 in dev_pair]
rest_set_1_ot=1*rest_set_1;rest_set_2_ot=1*rest_set_2
np.random.shuffle(rest_set_1)
np.random.shuffle(rest_set_2)
...
for turn in range(7):
    ...
    for i in trange(epoch):
        ...
        if i % 300 == 299:
            ...
            evaluater.test(Lvec, Rvec)
    if turn>0:
        Lvec, Rvec = get_embedding1(rest_set_1_ot, rest_set_2_ot)
        sim = spatial.distance.cdist(Lvec, Rvec, metric='cityblock')
        del Lvec;del Rvec;gc.collect()
        sim = torch.tensor(sim)
        dev_temp = []
        for i in range(len(rest_set_1_ot)):
            devtemp = []
            devtemp.append(rest_set_1_ot[i]);
            devtemp.append(rest_set_2_ot[i])
            dev_temp.append(devtemp)
        dev_temp = np.array(dev_temp);
        dev_temp = torch.tensor(dev_temp)
        tempair = get_hits_sinkhorn(test_pair=dev_temp, S=sim)
        del sim;del dev_temp;gc.collect()
    ...
    for a, b in enumerate(A):
        ...
        if B[b] == a and sim1>threashoud1:
            new_paircsls.append([rest_set_1[b], rest_set_2[a]])
    threashoud1 -= 0.05

    if turn>0:
        new_all = [x for x in new_pairot if x in new_paircsls]
    else:
        new_all=new_paircsls
    del vec;gc.collect()
    ...
    for e1, e2 in new_all:
        if e1 in rest_set_1:
            rest_set_1.remove(e1)
            if turn==0:
                rest_set_1_ot.remove(e1)

    for e1, e2 in new_all:
        if e2 in rest_set_2:
            rest_set_2.remove(e2)
            if turn==0:
                rest_set_2_ot.remove(e2)
    del new_paircsls;del new_pairot;del new_all;gc.collect()
