...
lang = 'zh'
train_pair,dev_pair,adj_matrix,r_index,r_val,adj_features,rel_features,triples1,triples2,noisy1,noisy2,entityid1,entityid2,triples_pos1,triples_pos2 ,entity1,entity2= load_data1('data/zh_en/',train_ratio=0.30)
...
def get_train_set(batch_size=batch_size):
    ...
    # negative_ratio=int(negative_ratio/2)+1
...
def get_trgat(node_size, rel_size, node_hidden, rel_hidden, triple_size, n_attn_heads=2, dropout_rate=0, gamma=3,
    ...
    alignment_input = Input(shape=(None, 6))
    ...
    def align_loss(tensor):
        ...
        l, r, fl, fr, sl, sr = [tensor[:, 0, :], tensor[:, 1, :], tensor[:, 2, :], tensor[:, 3, :], tensor[:, 4, :],
                                tensor[:, 5, :]]
        loss = K.relu(gamma + l1(l, r) - l1(l, fl)) + K.relu(gamma + l1(l, r) - l1(fr, r)) + K.relu(
            gamma + l1(l, r) - l1(l, sl)) + K.relu(gamma + l1(l, r) - l1(r, sr))
...
for i in trange(1200):
    ...
    if i==1199:
        ...
        for i in range(400):
            ...
            predt1 = net(martix_ne);predt2 = net(martix_net);predt3 = net(martix_net3);
        ...
        lensorthalf1 = int(lensorthalf * 0.75);
        tempsorthalf = 0
        x=0;y=0;
        ...
        print("鉴别结束。。。")
        adj_matrix, r_index, r_val, rel_matrix, ent_matrix,adj_features,rel_features = update('data/zh_en/', triples1, triples2)
