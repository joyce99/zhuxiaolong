...
#%%
lang = 'zh'
train_pair,dev_pair,adj_matrix,r_index,r_val,adj_features,rel_features,triples = load_dataOT('../RREA-Noisy/data/zh_en/',train_ratio=0.30,flag=False)
train_pairin=1*train_pair
...
def CSLS_test(thread_number=16, csls=10, accurate=True,flag=True,sim=None):
    vec = get_embedding()
    Lvec = np.array([vec[e1] for e1, e2 in dev_pair])
    Rvec = np.array([vec[e2] for e1, e2 in dev_pair])
    Lvec = Lvec / np.linalg.norm(Lvec, axis=-1, keepdims=True)
    Rvec = Rvec / np.linalg.norm(Rvec, axis=-1, keepdims=True)
    eval_alignment_by_sim_mat(Lvec, Rvec, [1, 5, 10], thread_number, csls=csls, accurate=accurate,flag=flag,sim=sim)
    return None
...
for turn in range(7):
    ...
    Rvec = Rvec / np.linalg.norm(Rvec, axis=-1, keepdims=True)
    sim = spatial.distance.cdist(Lvec, Rvec, metric='cityblock')
    sim = getdistance(sim, rest_set_1, rest_set_2, rel_type, train_pair, kg)
