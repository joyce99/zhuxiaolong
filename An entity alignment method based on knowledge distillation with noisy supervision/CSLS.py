...
def eval_alignment_by_sim_mat(embed1, embed2, top_k, nums_threads, csls=0, accurate=False,output = True,flag=True,sim=None):
    t = time.time()
    if flag:
        sim_mat = sim_handlerDis(embed1, embed2, csls, nums_threads,sim)
    else:
...
def sim_handlerDis(embed1, embed2, k, nums_threads,sim_mat):
    if sim_mat is None:
        sim_mat = -spatial.distance.cdist(embed1, embed2, metric='cityblock')
    else:
        print("Distance sim")
    if k <= 0:
        print("k = 0")
