...
#%%
lang = 'fr'
train_pair,dev_pair,adj_matrix,r_index,r_val,adj_features,rel_features = load_data('data/zh_en/',train_ratio=0.30)
...
def CSLS_test(thread_number=16, csls=10, accurate=True):
    ...
    eval_alignment_by_sim_mat(Lvec, Rvec, [1, 5, 10], thread_number, csls=csls, accurate=accurate,flag=False)
    return None
...
for turn in range(5):
    ...
    Lvec = Lvec / np.linalg.norm(Lvec, axis=-1, keepdims=True)
    Rvec = Rvec / np.linalg.norm(Rvec, axis=-1, keepdims=True)
    A, _ = eval_alignment_by_sim_mat(Lvec, Rvec, [1, 5, 10], 16, 10, True, False,flag=False)
    B, _ = eval_alignment_by_sim_mat(Rvec, Lvec, [1, 5, 10], 16, 10, True, False,flag=False)
    A = sorted(list(A));
    B = sorted(list(B))
    for a, b in A:
        if B[b][1] == a:
